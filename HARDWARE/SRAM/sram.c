...
#define BANK1_NE		3
#define Bank1_SRAM3_ADDR    ((u32)(0x60000000+0x04000000*(BANK1_NE-1)))	
...
void FSMC_SRAM_Init(void)
{
	...
	FSMC_Bank1->BTCR[2*BANK1_NE-2]=0X00000000;
	FSMC_Bank1->BTCR[2*BANK1_NE-1]=0X00000000;
	FSMC_Bank1E->BWTR[2*BANK1_NE-2]=0X00000000;
	//操作BCR寄存器	使用异步模式,模式A(读写共用一个时序寄存器)
	//BTCR[偶数]:BCR寄存器;BTCR[奇数]:BTR寄存器
	FSMC_Bank1->BTCR[2*BANK1_NE-2]|=1<<12;//存储器写使能
	FSMC_Bank1->BTCR[2*BANK1_NE-2]|=1<<4; //存储器数据宽度为16bit 	    
	//操作BTR寄存器			（HCLK=168M, 1个HCLK=6ns			    
	FSMC_Bank1->BTCR[2*BANK1_NE-1]|=8<<8; //数据保持时间（DATAST）为9个HCLK 6*9=54ns	 	 
	FSMC_Bank1->BTCR[2*BANK1_NE-1]|=0<<4; //地址保持时间（ADDHLD）未用到	  	 
	FSMC_Bank1->BTCR[2*BANK1_NE-1]|=0<<0; //地址建立时间（ADDSET）为0个HCLK 0ns	 	 
	//闪存写时序寄存器  
	FSMC_Bank1E->BWTR[2*BANK1_NE-2]=0x0FFFFFFF;//默认值
	//使能BANK1区域3
	FSMC_Bank1->BTCR[2*BANK1_NE-2]|=1<<0; 	
} 														  
