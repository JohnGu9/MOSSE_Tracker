...
//写寄存器函数
//regval:寄存器值
inline void LCD_WR_REG(volatile vu16 regval)
{   
...
//写LCD数据
//data:要写入的值
inline void LCD_WR_DATA(volatile vu16 data)
{	  
...
//LCD_Reg:寄存器地址
//LCD_RegValue:要写入的数据
inline void LCD_WriteReg(u16 LCD_Reg,u16 LCD_RegValue)
{	
...
//LCD_Reg:寄存器地址
//返回值:读到的数据
inline u16 LCD_ReadReg(u16 LCD_Reg)
{										   
...
//LCD写GRAM
//RGB_Code:颜色值
inline void LCD_WriteRAM(u16 RGB_Code)
{							    
...
//x,y:坐标
//返回值:此点的颜色
inline u16 LCD_ReadPoint(u16 x,u16 y)
{
 	static u16 r,g,b;
	r = 0; g = 0; b = 0;
...
//x,y:坐标
//color:颜色
inline static void LCD_Fast_DrawPoint(u16 x,u16 y,u16 color)
{	   
	if(lcddev.id==0X9341||lcddev.id==0X5310)
	{
		LCD_WR_REG(lcddev.setxcmd); 
...
//x1,y1:起点坐标
//x2,y2:终点坐标  
inline static void LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2)
{
	u16 t; 
	...
	{  
		...
		LCD_Fast_DrawPoint(uRow,uCol,POINT_COLOR);//快速画点
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
...
//画矩形	  
//(x1,y1),(x2,y2):矩形的对角坐标
inline void LCD_DrawRectangle(u16 x1, u16 y1, u16 x2, u16 y2)
{
...
//size:字体大小
//*p:字符串起始地址		  
inline void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
...
inline void LCD_DrawCross(u16 x1, u16 y1, u16 len)
{
	LCD_DrawLine(x1-len, y1, x1+len, y1);
	LCD_DrawLine(x1, y1-len, x1, y1+len);
}
